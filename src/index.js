...
// This file contains the main entry point for the command line `F1NFTAsset` app, and the command line option parsing code.
// See F1NFTAsset.js for the core functionality.
...
const {MakeF1NFTAsset} = require('./F1NFTAsset')
...
async function main() {
    ...
    program.command('deploy')
        .description('deploy an instance of the F1NFTAsset NFT contract')
        .option('-o, --output <deploy-file-path>', 'Path to write deployment info to', config.deploymentConfigFile || 'F1NFTAsset-deployment.json')
    ...
    // even if you call F1NFTAsset from elsewhere
...
async function createNFT(imagePath, options) {
    const F1NFTAsset = await MakeF1NFTAsset()
    ...
    const nft = await F1NFTAsset.createNFTFromAssetFile(imagePath, answers)
...
async function getNFT(tokenId, options) {
    ...
    const F1NFTAsset = await MakeF1NFTAsset()
    const nft = await F1NFTAsset.getNFT(tokenId, {fetchCreationInfo})
...
async function transferNFT(tokenId, toAddress) {
    ...
    await F1NFTAsset.transferToken(tokenId, toAddress)
    console.log(`🌿 Transferred token ${chalk.green(tokenId)} to ${chalk.yellow(toAddress)}`)
}

async function pinNFTData(tokenId) {
    const F1NFTAsset = await MakeF1NFTAsset()
    const {assetURI, metadataURI} = await F1NFTAsset.pinTokenData(tokenId)
