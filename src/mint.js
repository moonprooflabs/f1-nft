...
/**
 * Construct and asynchronously initialize a new F1NFTAsset instance.
 * @returns {Promise<F1NFTAsset>} a new instance of F1NFTAsset, ready to mint NFTs.
 */
 async function MakeF1NFTAsset() {
    const m = new F1NFTAsset()
    await m.init()
    return m
}

/**
 * F1NFTAsset is the main object responsible for storing NFT data and interacting with the smart contract.
 * Before constructing, make sure that the contract has been deployed and a deployment
 * info file exists (the default location is `F1NFTAsset-deployment.json`)
 * 
 * F1NFTAsset requires async initialization, so the F1NFTAsset class (and its constructor) are not exported. 
 * To make one, use the async {@link MakeF1NFTAsset} function.
 */
class F1NFTAsset {
    constructor() {
        this.ipfs = null
    ...
    async init() {
        ...
        // The F1NFTAsset object expects that the contract has already been deployed, with
        // details written to a deployment info file. The default location is `./F1NFTAsset-deployment.json`,
        // in the config.
    ...
    async _configurePinningService() {
        if (!config.pinningService) {
            throw new Error(`No pinningService set up in F1NFTAsset config. Unable to pin.`)
        }
